...
from models.layers.Discriminator import Discriminator
from utils.utils import adjust_learning_rate
from datasets.fusion import Fusion
from datasets.h36m import H36M
from datasets.h36m_pure import H36M_PURE
...
def main():
  ...
  DSmodel = Discriminator().cuda()

  DScriterion= torch.nn.BCELoss()


  DSoptimizer = torch.optim.RMSprop(DSmodel.parameters(), opt.LR,
                                  alpha = ref.alpha,
                                  eps = ref.epsilon,
                                  weight_decay = ref.weightDecay,
                                  momentum = ref.momentum)

  #DSoptimizer = torch.optim.Adam(DSmodel.parameters(), lr=0.0002)
  ...
  train_loader_3d =torch.utils.data.DataLoader(
      H36M_PURE(opt, 'train'),
  ...
  for epoch in range(1, opt.nEpochs + 1):
    loss_train, acc_train, mpjpe_train, loss3d_train = train(epoch, opt, train_loader ,train_loader_3d,model, criterion, DSmodel, DScriterion,optimizer, DSoptimizer)
