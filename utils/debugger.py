...
def show2D(img, points, c,filename):
  ...
  cv2.imwrite('../images/result/'+filename,img)
  return img
...
class Debugger(object):
  ...
  def show3D(self):
    ...
    plt.savefig('../images/result/'+self.filename.split('.')[0]+'_3d.png')
    #self.plt.show()
  ...
  def addPoint2D(self, point, c, imgId = 0):
    self.imgs[imgId] = show2D(self.imgs[imgId], point, c,self.filename)
