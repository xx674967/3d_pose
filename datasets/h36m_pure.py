...
class H36M_PURE(data.Dataset):
    def __init__(self, opt, split):
        ...
        f = File('/root/xz3/Toward-3d-pose/pytorch-pose-hg-3d/data/h36m/annotSampleTest.h5', 'r')
        for tag in tags:
        ...
        print 'Loaded 3D {} {} samples'.format(split, 22246)

    def LoadImage(self, index):
        folder = 's_{:02d}_act_{:02d}_subact_{:02d}_ca_{:02d}'.format(self.annot['subject'][index],
                                                                      self.annot['action'][index], \
                                                                      self.annot['subaction'][index],
                                                                      self.annot['camera'][index])
